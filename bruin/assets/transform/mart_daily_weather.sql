/* @bruin
name: marts.mart_daily_weather
type: duckdb.sql
connection: weather_warehouse
depends:
  - staging.stg_daily_observations
...
description: >
  Primary analytics table: daily weather observations joined with
  station metadata, enriched with rolling averages and extreme flags.
@bruin */

WITH observations AS (
    SELECT * FROM staging.stg_daily_observations
),

stations AS (
    SELECT * FROM staging.stg_stations
),
